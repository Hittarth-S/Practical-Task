...
const AddProductComponent = () => {
    ...
    const productId = useSelector((state) => (state?.user?.productId));
    ...
    const ProductDetailsComponent = () => {
                ...
                addProduct(values)
                    .then((res) => {
                        if (res?.status) {
                            ...
                            dispatch(actions.setProductId(res?.data));
                        } else {
                            toast.error(res?.message);
        ...
        return <form onSubmit={formikFunction.handleSubmit}>
    ...
    const ProductImageComponent = () => {
        ...
        const handleUpload = (e) => {
            if (file) {
                ...
                addProductImageByProductId(productDetails?._id, formData).then((res) => {
                    if (res?.status) {
                        toast.success(res?.message);
                        getProductDetailsFunction();
                        setKey('productImages');
        ...
        const handleDeleteImage = (deleteId) => {
            deleteProductImageByProductImageId(productDetails?._id, deleteId)
                .then((res) => {
                    if (res?.status) {
                        toast.success(res?.message);
                        getProductDetailsFunction();
